...
function SEOAnalyzer({ post, onKeywordsUpdate }) {
  ...
  const getScoreColor = (score) => {
    ...
    if (score >= 60) return 'text-yellow-600 dark:text-saffron'
        ...
        <div className="mt-2 bg-gray-200 dark:bg-gray-700 rounded-full h-2">
          <div
            className={`h-2 rounded-full transition-all ${
              ...
              analysis.score >= 60 ? 'bg-yellow-600' : 'bg-terra'
            }`}
      ...
      {showDetails && (
        <>
          ...
          {analysis.suggestions && analysis.suggestions.length > 0 && (
            <div>
              <h4 className="text-sm font-medium text-yellow-700 dark:text-saffron mb-2 flex items-center gap-1">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
